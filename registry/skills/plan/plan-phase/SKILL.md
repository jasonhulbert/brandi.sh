...
**Read the plan file contract first:** `{{SHARED_DIR}}/PLAN-CONTRACT.md` defines how to find the plan, the phase `Status:` values and who owns each transition, and the parent-index rules. The rules below are specific to *executing* a phase and assume that contract. If anything here seems to disagree with the contract, the contract wins.
